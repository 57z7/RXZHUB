...
local function createMobileButtons()
    ...
    local stackDefs = {
        { key = "aimbot", label = "AUTO BAT", row = 1, col = 1, cols = 3 },
        { key = "autoLeft", label = "AUTO LEFT", row = 1, col = 2, cols = 3 },
        { key = "autoRight", label = "AUTO RIGHT", row = 1, col = 3, cols = 3 },
        { key = "drop", label = "DROP", row = 2, col = 1, cols = 2 },
        { key = "batV2", label = "TP BAT", row = 2, col = 2, cols = 2 },
        { key = "tpDown", label = "TP DOWN", row = 3, col = 1, cols = 2 },
        { key = "carrySpeed", label = "CARRY SPD", row = 3, col = 2, cols = 2 },
        { key = "lagger", label = "LAGGER SPD", row = 4, col = 1, cols = 2 },
        { key = "laggerCarry", label = "LAG CARRY", row = 4, col = 2, cols = 2 },
    }

    local BTN_W, BTN_H, BTN_GAP, ROWS = 74, 66, 8, 4
    ...
    for i, def in ipairs(stackDefs) do
        ...
        local bStroke = mkStroke(btnFrame, Color3.fromRGB(255, 255, 255), 1)
        ...
        local function setOn(on)
            ...
            TS:Create(bStroke, TweenInfo.new(0.15), {
                Color = on and Color3.fromRGB(200, 200, 200) or Color3.fromRGB(255, 255, 255)
            }):Play()
        end
...
local function buildGui()
    ...
    Outer.Name = "Outer"
    Outer.Size = UDim2.new(0, 246, 0, 420)
    Outer.Position = UDim2.new(0, 20, 0, -60)
    ...
    Outer.Parent = GuiHub
    Instance.new("UICorner", Outer).CornerRadius = UDim.new(0, 22)
    ...
    HeaderSep.Position = UDim2.new(0, 12, 0, 36)
    ...
    miniIcon.Image = "https://www.roblox.com/asset-thumbnail/image?assetId=112701306083078&width=768&height=432&format=png"
...
settingsImage.Size = UDim2.new(1, 0, 0, 110)
settingsImage.BackgroundTransparency = 1
settingsImage.Image = "https://www.roblox.com/asset-thumbnail/image?assetId=112701306083078&width=768&height=432&format=png"
settingsImage.ScaleType = Enum.ScaleType.Crop
settingsImage.LayoutOrder = 0
settingsImage.Parent = mainScroll
